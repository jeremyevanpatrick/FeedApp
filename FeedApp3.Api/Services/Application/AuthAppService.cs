...
namespace FeedApp3.Api.Services.Application
{
    public class AuthAppService : IAuthAppService
    {
        ...
        {
            using (_logger.BeginLoggingScope(nameof(AuthAppService), nameof(SendEmailConfirmationEmail)))
            {
        ...
        {
            using (_logger.BeginLoggingScope(nameof(AuthAppService), nameof(SendPasswordResetEmail)))
            {
                string emailSubject = "Reset your password";

                var token = await _userManager.GeneratePasswordResetTokenAsync(user);
                var encodedToken = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(token));
                var resetUrl = $"{_clientSettings.Host}/login/resetpassword?email={Uri.EscapeDataString(email)}&token={Uri.EscapeDataString(encodedToken)}";
        ...
        {
            using (_logger.BeginLoggingScope(nameof(AuthAppService), nameof(SendChangeEmailEmail)))
            {
                ...
                var encodedToken = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(token));
                var confirmationUrl = $"{_clientSettings.Host}/login/confirmemailchange?userid={Uri.EscapeDataString(user.Id)}&email={Uri.EscapeDataString(newEmail)}&token={Uri.EscapeDataString(encodedToken)}";
        ...
        {
            using (_logger.BeginLoggingScope(nameof(AuthAppService), nameof(RefreshTokenAsync)))
            {
        ...
        {
            using (_logger.BeginLoggingScope(nameof(AuthAppService), nameof(LogoutAsync)))
            {
                if (string.IsNullOrWhiteSpace(refreshTokenString))
                {
                    return;
